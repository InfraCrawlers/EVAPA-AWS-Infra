...
def run_sync():
    ...
    with Gmp(connection=connection, transform=transform) as gmp:
        ...
        # Grab all reports (ignore pagination so we don't miss any)
        ...
        for report in reports_xml.xpath('//report'):
            ...
            # --- FIX 1: Check if the scan is actually finished ---
            status_list = report.xpath('./scan_run_status/text()')
            status = status_list[0] if status_list else "Unknown"

            if status != "Done":
                print(
                    f"Skipping report {report_id} from task '{task_name}' - Status is '{status}' (Waiting for 'Done')")
                continue
            ...
            if report_id not in sent_ids:
                ...
                try:
                    ...
                    # --- FIX 2: Correct filter to include ALL severities (Critical, High, Medium, Low, Log) ---
